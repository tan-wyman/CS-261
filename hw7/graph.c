...
int DFS(Graph* g, Vertex* source, Vertex* destination)
{
	...
	struct Vertex *tmp = source;
	initCirListDeque(stack);
	clearVisited(g);
	//add start vertex to stack
	addFrontCirListDeque(stack, tmp);
	//while stack is not empty, DFS
	while(!isEmptyCirListDeque(stack)){
		tmp = frontCirListDeque(stack);
		...
		for(int i = 0; i < tmp->numNeighbors; ++i){
			if(!tmp->neighbors[i]->isVisited)
				addFrontCirListDeque(stack, tmp->neighbors[i]);
		}
	}
	return 0;
}
