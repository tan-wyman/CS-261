...
void _adjustHeap(DynArr *heap, int max, int pos)
{
   /* FIXME */
	...
	int child = 0;
	if(right < max){
		child = _smallerIndexHeap(heap, left, right);
		if(compare(getDynArr(heap, pos), getDynArr(heap, child)) == 1){
			swapDynArr(heap, pos, child);
...
void _buildHeap(DynArr *heap)
{
    /* FIXME */
	assert(sizeDynArr(heap) > 0);
	for(int last = ((sizeDynArr(heap)/2) - 1); last > 0; --last)
		_adjustHeap(heap, (sizeDynArr(heap)), last);
	return;
}
/*
...
void sortHeap(DynArr *heap)
{
   /* FIXME */
	assert(sizeDynArr(heap) > 0);
	_buildHeap(heap);
	for(int last = sizeDynArr(heap) - 1; last > 0; last--){
