...
{
    /* FIXME: Write this */
	//struct DynArr *tmp = (struct DynArr *)malloc(sizeof(struct DynArr*));
	struct DynArr *tmp = createDynArr(sizeDynArr(heap));
	copyDynArr(heap, tmp);
	for(int i = 0; i < sizeDynArr(heap); ++i){
		sortHeap(tmp);
		print_type(getMinHeap(heap));
		removeMinHeap(tmp);
	}
	deleteList(tmp);
}
...
int compare(TYPE left, TYPE right)
{
	...
	if(getMinHeap(left) == getMinHeap(right))
		return 0;
	if(getMinHeap(left) < getMinHeap(right))
		return -1;
	if(getMinHeap(left) > getMinHeap(right))
